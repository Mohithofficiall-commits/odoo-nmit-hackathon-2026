...
import { Card, CardHeader, Button, Badge, Spinner, EmptyState, Avatar } from '@/components/ui';
...
export function EmployeeDashboard() {
  ...
  async function loadData() {
    ...
    const [todayRes, weekRes, leavesRes, payrollRes] = await Promise.all([
      supabase.from('attendance').select('*').eq('employee_id', profile!.id).eq('date', today).maybeSingle(),
      supabase.from('attendance').select('*').eq('employee_id', profile!.id).gte('date', weekAgo.toISOString().slice(0, 10)).order('date', { ascending: false }),
      supabase.from('leave_requests').select('*').eq('employee_id', profile!.id).order('created_at', { ascending: false }).limit(5),
  ...
  const presentDays = weekAttendance.filter((a) => a.status === 'present' || a.status === 'half_day').length;
  const totalWorkHours = weekAttendance.reduce((sum, a) => sum + (a.work_hours || 0), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-5">
          ...
          <p className="text-sm text-slate-500">Days present this week</p>
        </Card>
        ...
        <Card className="p-5">
          ...
          <p className="text-2xl font-bold text-slate-900">{totalWorkHours.toFixed(1)}h</p>
          <p className="text-sm text-slate-500">Working hours this week</p>
        </Card>
        ...
        <Card className="p-5">
          ...
          <p className="text-2xl font-bold text-slate-900">{leaveStats.pending}</p>
        ...
        <Card className="p-5">
          ...
          <p className="text-2xl font-bold text-slate-900">
            {payroll ? `$${payroll.net_salary.toLocaleString('en-US', { minimumFractionDigits: 0 })}` : '—'}
          </p>
          <p className="text-sm text-slate-500">Monthly net salary</p>
        </Card>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">Apply for Leave</p>
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">View Attendance</p>
              <p className="text-xs text-slate-500">Check your history</p>
            </div>
          </button>
          <button
          ...
          >
            ...
            <div>
              <p className="text-sm font-semibold text-slate-900">View Payroll</p>
      ...
      <Card>
        ...
        {recentLeaves.length === 0 ? (
          <EmptyState
            ...
            action={<Button size="sm" onClick={() => navigate('/leave')}>Apply for leave</Button>}
          />
        ) : (
              ...
              <div key={leave.id} className="flex items-center justify-between px-6 py-4">
                <div className="flex items-center gap-3">
                  <div className={`w-9 h-9 rounded-lg flex items-center justify-center ${
                    ...
                    {leave.leave_type === 'paid' ? <Wallet size={16} /> : leave.leave_type === 'sick' ? <XCircle size={16} /> : <CalendarDays size={16} />}
                  </div>
                  <div>
                    <p className="text-sm font-medium text-slate-900 capitalize">{leave.leave_type} leave</p>
